...
describe("changelog", () => {
  it("resuelve entradas y compara versiones", async () => {
    ...
    assert.ok(getChangelogEntry("1.4.4"));
    assert.ok(getChangelogEntry("1.4.3"));
    ...
    assert.ok(isVersionNewer("1.4.4", "1.4.3"));
    assert.ok(isVersionNewer("1.4.3", "1.4.2"));
    ...
    assert.ok(getChangelogSorted()[0].version === "1.4.4");
  });
});
